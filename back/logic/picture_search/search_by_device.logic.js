 ...
 */
'use strict';
var moduleName = 'search_by_device.logic';
...
function processRequest(param, fn){
    ...
    async.waterfall([
        ...
        function(result, next){
            var sqlstr = 'select deviceTele,id from ' + deviceProductModel.tableName;
            sqlstr += '  where (id in("' + globalIds.join('","') +'"))';
            sqlstr += ' ; ' ;
            var query = {
                sqlstr: sqlstr,
            };
                ...
                if (err) {
                    ...
                    for(var i=0;i<result.pics.length;i++){
                       for(var j=0;j<rows.length;j++){
                            if(rows[j].id == result.pics[i].deviceID){
                                result.pics[i].deviceTele = rows[j].deviceTele;
                            }
                        } 
                    }
                    next(null, result);
