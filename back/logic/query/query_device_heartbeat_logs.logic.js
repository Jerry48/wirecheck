...
'use strict';  
var moduleName = 'query_device_heartbeat_log.logic';
var URLPATH = '/v1/query/device/heartbeat/logs';
...
function processRequest(param, fn){
    ...
    async.waterfall([
        function(next){
            ...
            deviceHeartBeatModel.count(query, function(err, total){
                if (err) {
                    var msg = err.msg  || err;
                    console.error(moduleName + msg);
                    next(err);
        ...
        sqlstr += ' from '+deviceHeartBeatModel.tableName;
        if (param.deviceId) {
        ...
        deviceHeartBeatModel.query(query, function(err, rows){
            if (err) {
                var msg = err.msg  || err;
                console.error(moduleName + msg);
                next(err);
