// device self query group edit api
...
'use strict';  
var moduleName = 'query_self_edit.logic';
...
function processRequest(param, fn){
	...
	debug('Try to edit the device self query ' + id);
    	...
    	function(next){
    		deviceHelper.checkDeviceGroupExist(param, function(err, data){
                if (err) {
                    ...
                    if (!data.exist) {
                        var msg = 'Err: the device self query is not exist!';
                        console.error(moduleName + msg);
                        next({
    ...
    function(err){
    	if (err) {
    		console.error('Failed to edit device self query!'+id);
    		fn(err);
    	}else{
    		debug('Success to edit device self query'+id);
