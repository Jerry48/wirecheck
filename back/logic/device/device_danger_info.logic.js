...
'use strict';  
var moduleName = 'device_danger_info.logic';
var URLPATH = '/v1/device/danger/info';
...
function processRequest(param, fn){
    ...
    async.waterfall([
        function(next){
            var sqlstr = 'select id,dangerName';       
            sqlstr += ' from '+deviceDangerModel.tableName+' order by createTime DESC;';            
            var query = {
                sqlstr: sqlstr,
            };
            deviceDangerModel.query(query, function(err, rows){
                if (err) {
                    var msg = err.msg || err;
                    console.error(moduleName + msg);
                    next(err);
