...
'use strict';  
var moduleName = 'device_tree4.logic';
var URLPATH = '/v1/device/tree4';
...
function findParents(param,fn){
    var parentIds = [];
    var result = [];
    for (var i=0;i<param.length;i++){
        if(param[i].parentId=='root'){
            result.push(param[i]);
        }else{
            parentIds.push(param[i].parentId);                    
        }
    }
            ...
            if (err) {
                ...
                for(var i=0;i<tmp.length;i++){
                    ...
                    for(var j=0;j<param.length;j++){
                        if(param[j].parentId == tmp[i].id){
                            tmp[i].type = 1;
                            tmp[i].selectable=false;
                            tmp[i].num += param[j].num;
                            tmp[i].nodes.push(param[j]);
                        }
                    }
