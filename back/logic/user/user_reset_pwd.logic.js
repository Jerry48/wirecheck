// administrator reset user pwd api
...
'use strict';  
var moduleName = 'user_reset_pwd.logic';
...
function processRequest(param, fn){
	...
	debug('Try to reset user password:'+userName);
    ...
    async.waterfall([
    	...
    	function(user, next){
			...
			userModel.update(query, function(err,rows){
				if (err) {
					var msg = err.msg || err;
					console.error('Failed to reset the password:'+msg);
					next(err);
				}else {
					next(null, user);
				}
    ...
    function(err, user){
    	if (err) {
    		console.error('Failed to reset user password for'+userName);
    		fn(err);
    	}else{
    		debug('Success to reset user password for:'+userName);
