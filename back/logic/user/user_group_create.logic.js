...
'use strict';  
var moduleName = 'user_group_create.logic';
...
function processRequest(param, fn){
	...
	var groupId = '';
	debug('Try to create the group:'+groupName);
    ...
    async.series([
    	...
    	function(next){
    		groupId = dataHelper.createId(groupName);
    		var values = {
    			id: groupId,
    			name: groupName,
    		...
    		groupModel.create(query, next);
    	}
    ], 
    function(err){
    	if (err) {
    		console.error('Failed to create group for'+groupName);
    		fn(err);
    	}else{
    		debug('Success to create group for:'+groupName);
