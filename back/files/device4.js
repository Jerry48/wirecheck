...
var picserver = 'http://139.196.202.6:9090/';
var CANVASWIDTH = 640;
var ENTRIES = 50;
		...
		success: function(data) {
			if (data.code == 0) {
				...
				for (var i = 0; i < 50; i++) {
					...
					$("#table tbody").append("<tr id='" + list[i].deviceId + "' class='infolist'><td>" + (i + 1) + "</td><td>" + list[i].deviceName + "</td><td>" + list[i].deviceTele + "</td><td>" + list[i].area + "</td><td>" + list[i].lineName + "</td><td>" + list[i].danger + "</td>" + html + "<td>" + list[i].latitude + "</td><td>" + list[i].longitude + "</td><td>" + list[i].deviceMeid + "</td></tr>");
				}
			} else {
				alert('失败!');
			}
...
$(function() {
	...
	$('#modalAdd .modal-footer button:eq(0)').click(function() {
		if ($('#modalAdd-body input:eq(4)').val() != $('#modalAdd-body input:eq(5)').val()) {
			...
			if (checkUser(data) != 0) {
				...
				var danger = $('#modalAdd-body .selectDanger').find("option:selected").text();
	...
	$('#modalEdit .modal-footer button:eq(0)').click(function() {
		if ($('#modalEdit-body input:eq(3)').val() != $('#modalEdit-body input:eq(4)').val()) {
			...
			var data = {
				'userName': $('#modalEdit-body input:eq(3)').val(),
				'password': $('#modalEdit-body input:eq(4)').val(),
			};
			if (checkUser(data) != 0) {
				var deviceID = $('#modalEdit-body input:eq(0)').val();
				var deviceName = $('#modalEdit-body input:eq(1)').val();
				var deviceTele = $('#modalEdit-body input:eq(2)').val();
				...
				var area = area.join('/');
				var danger = $('#modalEdit-body .selectDanger').find("option:selected").text();
	...
	$('#channelSelect').change(function() {
		var deviceID = $('body').attr('hover-deviceId');
					...
					success: function(data) {
						if (data.code == 0) {
							...
							for (var i = 0; i < list.length; ++i) {
								var $ctrl = '<div><input filename="' + list[i] + '" type="radio">' + list[i] + '</div>';
	...
	$.ajax({
		...
		success: function(data) {
			if (data.code == 0) {
				...
				for (var i = 0; i < tmp; i++) {
					if (list[i].deviceDangerID == '') {
						list[i].deviceDangerID = 0;
					};
					switch (list[i].status) {
						case 0:
							var html = '<td style="color:red">离线</td>';
					...
					$("#table tbody").append("<tr id='" + list[i].deviceId + "' class='infolist' devicedangerid='" + list[i].deviceDangerID + "' lineid='" + list[i].lineId + "'><td>" + (i + 1) + "</td><td>" + list[i].deviceName + "</td><td>" + list[i].deviceTele + "</td><td>" + list[i].area + "</td><td>" + list[i].lineName + "</td><td>" + list[i].danger + "</td>" + html + "<td>" + list[i].latitude + "</td><td>" + list[i].longitude + "</td><td>" + list[i].deviceId + "</td></tr>");
				}
			} else {
				alert('获取设备信息列表失败');
			}
	...
	$.ajax({
		...
		success: function(data) {
			if (data.code == 0) {
				...
				for (var i = 0; i < 50; i++) {
					var html1 = "<a href = '#' style = 'margin-left:10px;' onmouseout='hiddenPic();' onmousemove='showPic(event,\"" + list[i].thumbnailPicUrl1 + "\");'>摄像头1</a>";
					var html2 = "<a href = '#' style = 'margin-left:10px;' onmouseout='hiddenPic();' onmousemove='showPic(event,\"" + list[i].thumbnailPicUrl2 + "\");'>摄像头2</a>";
					var html3 = "<a href = '#' style = 'margin-left:10px;' onmouseout='hiddenPic();' onmousemove='showPic(event,\"" + list[i].thumbnailPicUrl3 + "\");'>摄像头3</a>";
					var htmls = ''
...
function selectLevels() {
	...
	var parentId = $(".selectLevel0").find("option:selected").attr('value');
	var data = {
		'parentId': parentId,
		'level': 1,
	};
	...
	$.ajax({
		...
		success: function(data) {
			if (data.code == 0) {
				...
				$('#myCanvas').css('background-image', 'url(' + picserver + result.path + ')');
