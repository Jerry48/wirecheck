...
$(function () {
    ...
    $('#searchMap').click(function () {
        ...
        $.ajax({
            ...
            if (data.status === 0) {
                var _address = data.result.location;
                var data = {
                    'latitude': parseFloat(_address.lat),
                    'longitude': parseFloat(_address.lng),
    ...
    function channelLineTree() {
        var rootNode = void 0;
        ajxSync('/v1/device/tree/channel/line', 'POST', { userId: cookie_userId }).then(function (data) {
            if (data.code == 0) {
                rootNode = data.result.data;
            } else {
                console.log('获取设备树失败');
