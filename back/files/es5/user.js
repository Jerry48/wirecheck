...
$(function () {
    ...
    $('#modalAdd .modal-footer button:eq(0)').click(function () {
        ...
        if (userType == 1 || $('#pselectAll').prop('checked')) {
            ...
            if ($('#pselectOne input:eq(0)').prop('checked')) {
                usrEdit = 1;
            } else {
                usrEdit = 0;
            }
            if ($('#pselectOne input:eq(1)').prop('checked')) {
                pwdEdit = 1;
            } else {
                pwdEdit = 0;
            }
            if ($('#pselectOne input:eq(2)').prop('checked')) {
                deviceOp = 1;
            } else {
                deviceOp = 0;
            }
            if ($('#pselectOne input:eq(3)').prop('checked')) {
                channelSet = 1;
            } else {
                channelSet = 0;
            }
            if ($('#pselectOne input:eq(4)').prop('checked')) {
                wechatPush = 1;
            } else {
                wechatPush = 0;
            }
            if ($('#pselectOne input:eq(5)').prop('checked')) {
                createGroup = 1;
            } else {
                createGroup = 0;
            }
            ...
            success: function success(data) {
                if (data.code == 0) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        $("#main table tbody").append("<tr id='" + list[i].userId + "' class='infolist' usertype='" + list[i].userType + "' department='" + list[i].department + "' mobile='" + list[i].mobile + "' groupid='" + list[i].groupId + "'><td><input type='checkbox' name=''></td><td>" + (i + 1) + "</td><td>" + list[i].name + "</td><td>" + list[i].userName + "</td><td>" + userType + "</td><td>" + groupName + "</td><td>" + list[i].department + "</td><td>" + list[i].mobile + "</td>");
                    }
                    if (list.length < 35) {
