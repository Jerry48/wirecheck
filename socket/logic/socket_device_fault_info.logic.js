 ...
 */
'use strict';  
var moduleName = 'socket_device_fault_info.logic';
var URLPATH = '/v1/device/faultInfo';
...
function processPayload(packet){
    ...
    var pldata={
        ...
        fault_desc : dataBuf.slice(4,len).toString('ascii').trim(), 
    };
    debug('the timestamp:' + pldata.time_stamp);
    return pldata;
}
...
function processRequest(packet, fn){
    ...
    async.series([
    	function(next){
            var id = dataHelper.createId(pldata.time_stamp);
            var values = {
               deviceID : deviceId,
               faultDesc : pldata.fault_desc,
               state : 0
            }
            var query = {
                fields: values,
            ...
            deviceFaultModel.create(query, next);
        } ], 
    function(err, device){
        if (err) {
    		console.error('Failed to create the fault info for device:' + deviceId);
    		//fn(err);
    	}else{
    		debug('Success to create the fault info for device:' + deviceId);
